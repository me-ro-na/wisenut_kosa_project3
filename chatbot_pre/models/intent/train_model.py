...
# 데이터 읽어오기
train_file = rf"{os.path.abspath('chatbot_pre/models/intent/total_train_questions.csv')}"
data = pd.read_csv(train_file)
queries = data['Questions'].tolist()
intents = data['Labels'].tolist()

import sys
[sys.path.append(i) for i in ['.', '..']]
from chatbot_pre.utils.Preprocess import Preprocess
p = Preprocess(word2index_dic=rf'{os.path.abspath("chatot_pre/train_tools/dict/chatbot_dict.bin")}')
...
# 모델 저장  ○8
model.save(rf'{os.path.abspath("chatbot_pre/models/intent/intent_model.h5")}')
